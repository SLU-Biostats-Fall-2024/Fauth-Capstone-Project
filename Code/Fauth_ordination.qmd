...
```{r}
ord_count <- scount |> group_by(Board_Number,Species, Site_Type) |> summarise(total = sum(Count))
head(ord_count,12)
```
...
```{r}
SalOrd <- metaMDS(wide_df[,3:7], distance = "bray", k = 2)
```
